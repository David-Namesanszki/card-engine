...
LeftHandUI::LeftHandUI(Animator& animator, LeftHandUIConfig config)
...
void LeftHandUI::addCard(CardUI card) {
    ...
    for (auto& c : _cards) {
        if (_config.shiftAnimDef && slot < _config.slotCount) {
            Animation anim = _config.shiftAnimDef(c.transform, _slots[slot]);
            anim.tag = c.id();
    ...
    CardUI& front = _cards.front();

    if (_config.addAnimDef) {
        Animation anim = _config.addAnimDef(front.transform, _slots[0]);
        anim.tag = front.id();
        _animator.add(std::move(anim));
    } else {
        applyAnchor(front.transform, _slots[0]);
    }
}
...
void LeftHandUI::applyAnchor(TransformComponent& t, const AnchorPoint& anchor) {
...
std::list<CardUI>::iterator LeftHandUI::findCard(uint32_t id) {
    return std::find_if(_cards.begin(), _cards.end(), [id](const CardUI& c) {
        return c.id() == id;
    });
}
