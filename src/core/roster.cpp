...
void Roster::addUnit(Unit unit) {
    if (_boardPieces.size() >= _maxSize)
        return;
    _boardPieces.push_back(std::make_unique<Unit>(std::move(unit)));
}

void Roster::addConstruction(Construction construction) {
    if (_boardPieces.size() >= _maxSize)
        return;
    _boardPieces.push_back(std::make_unique<Construction>(std::move(construction)));
}
